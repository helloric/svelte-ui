...
We distinguish between pauses and stopped speech by using something that we call a "pause threshold". Some amount of seconds that has to pass until we consider non-speaking parts not as a pause, but as stopped speaking altogether. We do so by collecting frames below the decibel threshold into an extra-collection called `idle_frames`. We clear this collection everytime speaking is heard. This collection eventually accumulates over time if no speaking is done for a while.
